...
def _tier(score: float, *, evidence_missing: bool = False) -> str:
    if score >= 0.75:
        return "medium" if evidence_missing else "high"
    if score >= 0.55:
        return "medium"
    if score >= 0.35:
        return "low"
...
def candidate_quality(input_smiles: str, candidate: dict[str, Any], module_name: str = "") -> dict[str, Any]:
    ...
    if evidence_missing:
        ...
        score -= 0.25
        if candidate_source.startswith("microbial_rescue:"):
            flags.append(
                QualityFlag(
                    "heuristic_microbial_rescue",
                    "info",
                    "Candidate was generated by broad microbial rescue SMARTS; treat it as a pathway hypothesis until organism, enzyme, or literature evidence is attached.",
                )
            )
            score -= 0.05
        if "phenol_dehydroxylation" in candidate_source:
            flags.append(
                QualityFlag(
                    "broad_dehydroxylation_hypothesis",
                    "warning",
                    "Phenolic dehydroxylation is gut-microbe plausible but position-, substrate-, and strain-dependent.",
                )
            )
            score -= 0.10
    ...
    tier = _tier(score, evidence_missing=evidence_missing)
    ...
    result.update(
        {
            ...
            "evidence_level": "structural_only" if evidence_missing else "traceable_biochemical_evidence",
...
def annotate_prediction_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in out.get("top", []):
        ...
        evidence_level = row["biochem_quality"].get("evidence_level", "unknown")
        tier_counts[tier] = tier_counts.get(tier, 0) + 1
        evidence_level_counts[evidence_level] = evidence_level_counts.get(evidence_level, 0) + 1
    ...
    out["application_boundary"] = {
        ...
        "quality_meaning": "Biochemical quality flags support review; high tier requires traceable evidence, while structural-only candidates are capped at medium and do not prove occurrence.",
