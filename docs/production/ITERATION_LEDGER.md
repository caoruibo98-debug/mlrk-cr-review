...
| 07 | `generation_07` | complete | Can evaluation reveal the next production gap beyond glycoside hydrolysis? | Added challenge panel, production scorecard, no-candidate payload fix, reviewer README, dependency declarations, and `scripts/` entrypoints. | Challenge panel top-5 hit rate is `0 / 6`; next round must expand sample size and failure taxonomy. |
| 08 | pending | planned | Can the challenge panel be expanded to at least 20 literature-backed cases? | Not started. | Needs curated food-metabolite sample expansion. |
| 09 | pending | planned | Can external tool inputs be exported reproducibly? | Not started. | Needs BioTransformer/MicrobeRX/GutBug-compatible exports. |
| 10 | pending | planned | Can generation recall and ranking recall be separated for every panel? | Not started. | Needs per-case failure taxonomy. |
| 11 | pending | planned | Can ablations isolate chemistry-only, EC-only, Tanimoto, and rescue effects? | Not started. | Needs rescue-off and evidence-off switches. |
