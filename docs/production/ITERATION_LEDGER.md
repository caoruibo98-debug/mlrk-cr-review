...
| 17 | `generation_17` | complete | Can model cards and claim boundaries be tied to scorecard outputs? | Added scorecard-backed model-card generator, `docs/MODEL_CARD.md`, summary JSON, tests, repo-doctor integration, and README/manifest links. | Model card still reflects internal evidence only until external outputs or wet-lab evidence are imported. |
| 18 | `generation_18` | complete | Can CodeRabbit or equivalent review be run on the production branch after fixes? | CodeRabbit CLI and auth were confirmed, but `coderabbit review --agent --base origin/master` timed out after `604046 ms`; added external review status gate and tests so timeout cannot be claimed as a passed review. | Needs a successful external AI/code review run when CodeRabbit is responsive. |
| 19 | `generation_19` | complete | Can a release candidate pass fresh clone, tests, appraisal, and scorecard? | Fresh-cloned GitHub tag `generation_18` at `fda7f6c`, passed 49 contract tests, scorecard, readiness, repo doctor, and model card; added fresh-clone report tool/artifact/tests. | Final generation still needs a consolidated internal/external/cross/ablation release summary. |
| 20 | `generation_20` | complete | Can the final GitHub version justify its production boundary? | Added atomic report writes, final release summary JSON/Markdown, tests for release lanes and atomic writes, and updated README/manifest/repo doctor. | External tool outputs, wet-lab validation, and successful CodeRabbit review remain future blockers beyond this internal MVP. |
